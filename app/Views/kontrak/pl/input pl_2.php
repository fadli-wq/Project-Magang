...
<div class="d-flex">
    ...
    <div class="container-fluid p-4">
        <h2 class="text-center">Input Pembayaran PL</h2>
        <div class="form-container">
            <form action="<?= base_url('kontrak/pl/pembayaran/termin_simpan_session') ?>" method="post">
                <?= csrf_field() ?>
                
                <div class="mb-3">
                    <label class="form-label">Pagu</label>
                    <input type="number" name="pagu" class="form-control" placeholder="Masukkan Pagu" value="<?= session()->get('pl_pembayaran.pagu') ?? '' ?>" required>
                </div>

                <div class="mb-3">
                    <label class="form-label">Metode Pembayaran</label>
                    <select name="metode" class="form-control" required>
                        <option value="Sekaligus" <?= (session()->get('pl_pembayaran.metode') == 'Sekaligus') ? 'selected' : '' ?>>Sekaligus</option>
                        <option value="Termin" <?= (session()->get('pl_pembayaran.metode') == 'Termin') ? 'selected' : '' ?>>Termin</option>
                    </select>
                </div>

                <div class="mb-3">
                    <label class="form-label">Jumlah Termin</label>
                    <input type="number" name="jumlah_termin" class="form-control" placeholder="Masukkan Jumlah Termin" value="<?= session()->get('pl_pembayaran.jumlah_termin') ?? '1' ?>" required>
                </div>

                <div class="mb-3">
                    <label class="form-label">Sumber Dana</label>
                    <input type="text" name="sumber_dana" class="form-control" placeholder="Masukkan Sumber Dana" value="<?= session()->get('pl_pembayaran.sumber_dana') ?? '' ?>" required>
                </div>

                <div class="d-flex justify-content-between">
                    <a href="<?= base_url('kontrak/pl') ?>" class="btn btn-secondary mt-3">Back</a>
